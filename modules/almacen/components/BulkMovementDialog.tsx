...
export function BulkMovementDialog({ products, type, onComplete, mode = "DIRECT", branch = "Principal" }: { products: Product[]; type: MovementType; onComplete: () => void; mode?: "DIRECT" | "REQUEST"; branch?: string }) {
  ...
  const [selected, setSelected] = useState<Array<{ product: Product; units: number; measure: "boxes" | "units" }>>([]);
  ...
  const add = (product: Product) => {
    ...
    setSelected((items) => [...items, { product, units: 1, measure: product.boxes > 0 ? "boxes" : "units" }]);
  };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (mode === "DIRECT") setVoucher({ ...result.data, items: selected.map((item) => ({ ...item.product, unitsCount: item.measure === "boxes" ? item.units * (item.product.unitsPerBox || 1) : item.units, quantity: item.units, measure: item.measure })) });
    close();
    onComplete();
  };
        ...
        <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden">
          ...
          <form onSubmit={submit} className="p-6 space-y-4">
            ...
            <div className="relative"><Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar código, producto o marca..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs" />
              {search && <div className="absolute z-10 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg max-h-44 overflow-y-auto">{filtered.map((p) => <button type="button" key={p.id} onClick={() => add(p)} className="w-full px-3 py-2 text-left hover:bg-slate-50 text-xs flex justify-between"><span><strong>{p.code}</strong> {p.name}</span><span className="text-slate-500">{p.boxes > 0 ? `${p.boxes} cajas` : `${p.looseUnits || 0} sueltas`}</span></button>)}</div>}
            </div>
            <div className="space-y-2 max-h-56 overflow-y-auto">{selected.map((item) => <div key={item.product.id} className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl"><div className="flex-1 min-w-0"><p className="text-xs font-bold text-slate-800 truncate">{item.product.name}</p><p className="text-[11px] text-slate-500">{item.measure === "boxes" ? `${item.product.boxes} cajas · ${item.product.unitsPerBox || 1} uds/caja` : `${item.product.looseUnits || 0} unidades sueltas`}</p></div><select value={item.measure} onChange={(e) => setSelected((items) => items.map((entry) => entry.product.id === item.product.id ? { ...entry, measure: e.target.value as "boxes" | "units" } : entry))} className="rounded-lg border border-slate-200 px-2 py-2 text-xs font-semibold"><option value="boxes">Cajas</option><option value="units">Unidades</option></select><input type="number" min={1} value={item.units} onChange={(e) => updateUnits(item.product.id, Number(e.target.value))} className="w-20 px-2 py-2 text-xs font-bold border border-slate-200 rounded-lg" /><button type="button" onClick={() => setSelected((items) => items.filter((entry) => entry.product.id !== item.product.id))} className="text-red-500"><Trash2 className="w-4 h-4" /></button></div>)}{selected.length === 0 && <p className="p-5 text-center text-xs text-slate-400">Busque y agregue los modelos que forman parte del movimiento.</p>}</div>
