...
export function BulkMovementDialog({ products, type, onComplete }: { products: Product[]; type: MovementType; onComplete: () => void }) {
  ...
  const [reason, setReason] = useState(type === "ENTRY" ? "RecepciÃ³n de mercancÃ­a" : "Despacho de mercancÃ­a");
    ...
    <>
      ...
      {open && <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between"><h2 className="font-bold text-slate-800">Nueva {type === "ENTRY" ? "entrada" : "salida"} de mercancÃ­a</h2><button onClick={close} className="text-slate-400"><X /></button></div>
          <form onSubmit={submit} className="p-6 space-y-4">
            {error && <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-xs font-semibold text-red-700">{error}</div>}
            <div className="relative"><Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar cÃ³digo, producto o marca..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs" />
