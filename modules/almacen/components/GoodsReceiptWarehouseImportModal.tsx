...
function buildLines(receipt: ImportReceipt): ImportLine[] {
  return (receipt.items || []).flatMap((item) => {
    ...
    return variants.map((variant) => ({ itemId: item.id || "", code: item.code || "", name: variant.model?.trim() || item.model?.trim() || parsed.model, brand: variant.brand?.trim() || item.brand?.trim() || parsed.brand, capacity: variant.capacity?.trim() || item.capacity?.trim() || parsed.capacity, color: variant.color?.trim() || "", quantity: Number(variant.quantity) || Number(item.quantity) || 1, unitsPerBox: 1 }));
  });
}
