...
import { Boxes, Building2, CalendarDays, CheckCircle2, ClipboardList, Eye, PackagePlus, Plus, Printer, RefreshCw, Search, Send, Share2, UserRound, X, XCircle } from "lucide-react";
        ...
        <div>
          ...
          <div className="mt-3 overflow-hidden rounded-xl border border-slate-200">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">{items.map((item: any) => <tr key={item.id} className="hover:bg-slate-50/70"><td className="px-4 py-3 font-mono text-xs font-bold text-[#5750f1]">{item.product?.code ?? "—"}</td><td className="px-4 py-3 font-semibold text-slate-800">{item.product?.name ?? item.productId}</td><td className="px-4 py-3 text-xs text-slate-600">{formatItemPresentation(item, detail.itemsMap[item.productId])}</td><td className="px-4 py-3 text-right font-bold text-slate-800">{item.unitsCount} uds</td></tr>)}</tbody>
            </table>
