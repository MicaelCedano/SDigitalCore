...
export async function getWorkCenterData() {
  ...
  const tasks = await prisma.workTask.findMany({
    where: scope,
    include: { creator: { select: { name: true, email: true } }, assignee: { select: { id: true, name: true, email: true } }, assignees: { include: { user: { select: { id: true, name: true, email: true } } }, orderBy: { assignedAt: "asc" } }, events: { orderBy: { createdAt: "desc" }, take: 8, include: { actor: { select: { name: true } } } } },
    orderBy: [{ status: "asc" }, { dueAt: "asc" }, { priority: "desc" }, { createdAt: "desc" }],
    take: 100,
  });
  const [activeUsers, completedToday, waiting] = await Promise.all([
    prisma.user.findMany({ where: { status: "ACTIVE", allowedModules: { has: "centro-trabajo" } }, select: { id: true, name: true, email: true }, orderBy: { name: "asc" }, take: 100 }),
