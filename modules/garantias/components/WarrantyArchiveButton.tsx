    ...
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="warranty-archive-title">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between">
          <div><p className={`text-xs font-bold uppercase tracking-[0.15em] ${archived ? "text-emerald-600" : "text-red-600"}`}>{archived ? "Volver al panel" : "Retirar caso"}</p><h2 id="warranty-archive-title" className="mt-2 text-xl font-black text-slate-800">¿{archived ? "Restaurar" : "Archivar"} {caseCode}?</h2><p className="mt-2 text-sm text-slate-500">{archived ? "El caso volverá a aparecer en el flujo operativo." : "Se ocultará de los casos activos, pero conservará su historial y documentos."}</p></div>
          <button type="button" onClick={() => setOpen(false)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Cerrar"><X size={18} /></button>
        </div>
        <label className="mt-5 block text-sm font-semibold text-slate-700">{archived ? "Observación (opcional)" : "Motivo"}<textarea required={!archived} maxLength={1000} value={reason} onChange={(event) => setReason(event.target.value)} placeholder={archived ? "Ej. Registro revisado y habilitado" : "Ej. Registro duplicado"} className="mt-2 min-h-24 w-full rounded-xl border border-slate-300 p-3 text-sm outline-none focus:border-red-500 focus:ring-4 focus:ring-red-500/10" /></label>
        {error && <p role="alert" className="mt-3 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}
        <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setOpen(false)} disabled={busy} className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700">Cancelar</button><button type="button" onClick={submit} disabled={busy} className={`rounded-xl px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50 ${archived ? "bg-emerald-600" : "bg-red-600"}`}>{busy ? "Procesando..." : archived ? "Restaurar caso" : "Archivar caso"}</button></div>
      </div>
    </div>}
  </>;
}
