...
export function WarrantyFlow({ operation, cases, embedded = false }: { operation: WarrantyFlowOperation; cases: WarrantyFlowCase[]; embedded?: boolean }) {
  ...
  return <>
    <section className="rounded-2xl border border-slate-200 bg-white shadow-2xs overflow-hidden">
      ...
      {operation !== "credit" && <div className="grid gap-4 border-b border-slate-200 p-5 sm:grid-cols-2 sm:p-6"><label className="text-sm font-semibold text-slate-700">{label}<input ref={operation === "sendSupplier" ? supplierInputRef : undefined} value={counterparty} onChange={(event) => setCounterparty(event.target.value)} placeholder={operation === "assign" ? "Ej. Sahul" : "Escriba o seleccione..."} list={operation === "sendSupplier" ? "warranty-supplier-suggestions" : undefined} disabled={operation === "sendSupplier" && supplierLocked} className="mt-2 h-11 w-full rounded-xl border border-slate-300 px-3 outline-none transition focus:border-[#5750f1] focus:ring-2 focus:ring-[#5750f1]/10 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500" />{operation === "sendSupplier" && <datalist id="warranty-supplier-suggestions">{SUPPLIER_SUGGESTIONS.map((supplier) => <option key={supplier} value={supplier} />)}</datalist>}{operation === "sendSupplier" && supplierLocked && <div className="mt-2 flex items-center justify-between gap-2"><span className="text-[11px] font-semibold text-amber-700">Destino bloqueado para este envío.</span><button type="button" onClick={() => { setSupplierLocked(false); window.setTimeout(() => supplierInputRef.current?.focus(), 0); }} className="text-[11px] font-bold text-red-600 underline">Editar destino</button></div>}</label>{operation === "receiveTech" && <label className="flex items-center gap-3 self-end pb-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={repaired} onChange={(event) => setRepaired(event.target.checked)} className="h-4 w-4 accent-[#5750f1]" /> Equipo reparado</label>}</div>}
