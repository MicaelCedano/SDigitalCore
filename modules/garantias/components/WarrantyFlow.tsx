...
export function WarrantyFlow({ operation, cases, embedded = false }: { operation: WarrantyFlowOperation; cases: WarrantyFlowCase[]; embedded?: boolean }) {
  ...
  function selectBulk() {
    const tokens = bulkInput.split(/[\s,;]+/).map((value) => value.trim()).filter(Boolean);
    if (tokens.length === 0) return setBulkMessage("Pega al menos un IMEI o código de garantía.");
    const normalized = new Set(tokens.map((value) => value.toLowerCase()));
    const matches = cases.filter((item) => normalized.has(item.imei.toLowerCase()) || normalized.has(item.caseCode.toLowerCase()));
    const unmatched = tokens.filter((value) => !cases.some((item) => item.imei.toLowerCase() === value.toLowerCase() || item.caseCode.toLowerCase() === value.toLowerCase()));
    setSelected((current) => [...new Set([...current, ...matches.map((item) => item.caseCode)])]);
