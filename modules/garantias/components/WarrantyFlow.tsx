...
export function WarrantyFlow({ operation, cases }: { operation: WarrantyFlowOperation; cases: WarrantyFlowCase[] }) {
  ...
  return <>
    <section className="rounded-2xl border border-slate-200 bg-white shadow-2xs overflow-hidden">
      ...
      <div className="flex flex-col gap-3 border-t border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><span className="text-xs text-slate-500">Puedes revisar los datos antes de confirmar.</span><button type="button" disabled={busy || loadingDocument || selected.length === 0} onClick={validateBeforeConfirm} className="rounded-xl bg-[#5750f1] px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#463ec5] disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Procesando..." : loadingDocument ? "Abriendo documento..." : actionLabel}</button></div>
