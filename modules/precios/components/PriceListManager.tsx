...
import {
  Download, Edit, GripVertical, Image as ImageIcon, ListPlus, Loader2, Pencil,
  Plus, Search, Settings2, Trash2, Upload, X,
} from "lucide-react";
import {
...
export function PriceListManager() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5 pb-10">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 bg-white p-4"><div><h2 className="font-black text-slate-900">Vista previa HD</h2><p className="text-xs text-slate-500">1080 px · agrupada por marca y ordenada por precio</p></div><ListPlus className="h-5 w-5 text-indigo-600" /></div><div className="max-h-[880px] overflow-auto p-4"><div ref={previewRef} style={{ backgroundColor: "#ffffff" }} className="mx-auto w-[1080px] min-h-[1000px] bg-white text-black shadow-xl"><div style={{ backgroundColor: "#ffffff" }} className="flex h-48 items-center justify-center border-b border-slate-100 p-4">{logo ? <img src={logo} alt="Logo de SDigital" className="max-h-full max-w-full object-contain" /> : <div className="flex h-full w-full flex-col items-center justify-center bg-slate-50 text-slate-300"><ImageIcon className="h-12 w-12" /><span className="mt-2 text-xl font-black tracking-widest">[ LOGO ]</span></div>}</div><div style={{ backgroundColor: "#ffffff" }} className="columns-2 gap-8 p-8">{grouped.map(({ brand, items }) => <div key={brand.id} style={{ backgroundColor: "#ffffff" }} className="mb-8 break-inside-avoid overflow-hidden rounded-xl border border-slate-100"><div style={{ backgroundColor: brand.color }} className="px-6 py-3 text-center text-2xl font-black uppercase tracking-wider text-white">{brand.name}</div><div style={{ backgroundColor: "#ffffff" }} className="p-5">{items.map((item) => <div key={item.id} className="flex items-baseline justify-between gap-3 border-b border-slate-100 py-2.5 last:border-0"><span className="text-xl font-black">{item.model} <small className="text-base font-medium text-slate-500">{item.capacity}</small></span><span className="whitespace-nowrap text-xl font-black">{money(item.retailPrice)}</span></div>)}</div></div>)}</div>{!activeList.length && <div className="p-20 text-center text-3xl italic text-slate-300">Lista de precios vacía</div>}<div style={{ backgroundColor: "#ffffff" }} className="mt-auto border-t border-slate-100 px-12 py-8 text-center text-base text-slate-400">Generado automáticamente · {new Date().toLocaleDateString("es-DO")}</div></div></div></section>
    </div>
