...
type Props = { initialShipments: Shipment[]; drivers: Driver[]; currentUserId: string };

export function ShipmentsManager({ initialShipments, drivers, currentUserId }: Props) {
  const [shipments, setShipments] = useState(initialShipments);
  const [selectedId, setSelectedId] = useState(initialShipments[0]?.id ?? null);
  const [form, setForm] = useState({ title: "", origin: "", destination: "", vehicleLabel: "", driverId: "", notes: "" });
  ...
  const createShipment = async (event: React.FormEvent) => {
    ...
    setShipments((current) => [data.shipment, ...current]); setSelectedId(data.shipment.id); setForm({ title: "", origin: "", destination: "", vehicleLabel: "", driverId: "", notes: "" });
  };
