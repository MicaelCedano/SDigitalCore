...
function formatDateTime(value: string | null) {
  return value ? new Intl.DateTimeFormat("es-DO", { dateStyle: "medium", timeStyle: "short", timeZone: "America/Santo_Domingo" }).format(new Date(value)) : "Sin registrar";
}
...
export function ShipmentsManager({ initialShipments, drivers, addresses, currentUserId }: Props) {
  ...
  return <main className="mx-auto max-w-[1500px] space-y-5">
    ...
    <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm"><button type="button" onClick={() => { setView("active"); setSelectedId(shipments[0]?.id ?? null); }} className={`rounded-xl px-4 py-2 text-sm font-bold ${view === "active" ? "bg-slate-900 text-white" : "text-slate-600"}`}>En curso ({shipments.length})</button><button type="button" onClick={() => setView("history")} className={`rounded-xl px-4 py-2 text-sm font-bold ${view === "history" ? "bg-indigo-600 text-white" : "text-slate-600"}`}>Historial ({historyShipments?.length ?? "…"})</button></div>
    <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
      <section className="space-y-4">
        ...
        <div className="space-y-2">{(view === "history" && historyShipments === null ? [] : visibleShipments).map((shipment) => <button type="button" key={shipment.id} onClick={() => setSelectedId(shipment.id)} className={`w-full rounded-2xl border p-4 text-left transition ${selectedId === shipment.id ? "border-indigo-400 bg-indigo-50" : "border-slate-200 bg-white hover:border-indigo-200"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-xs font-bold text-indigo-600">{shipment.code}</p><p className="mt-1 font-bold text-slate-900">{shipment.title}</p><p className="mt-1 text-xs text-slate-500">{shipment.stops.length > 0 ? `${shipment.stops.length} paradas` : shipment.destination}</p>{view === "history" ? <p className="mt-2 text-xs font-semibold text-slate-500">{shipment.status === "DELIVERED" ? `Completado: ${formatDateTime(shipment.deliveredAt)}` : `Cancelado · ${formatDateTime(shipment.deliveredAt)}`}</p> : null}</div><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-600">{statusLabels[shipment.status]}</span></div></button>)}{view === "history" && historyShipments === null ? <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500">Cargando historial…</div> : null}{historyShipments !== null && view === "history" && visibleShipments.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500">Todavía no hay envíos terminados.</div> : null}{view === "active" && shipments.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500">Todavía no hay envíos activos.</div> : null}</div>
      </section>
      <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">{selected ? <>
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs font-bold text-indigo-600">{selected.code}</p><h2 className="mt-1 text-xl font-black text-slate-900">{selected.title}</h2><p className="mt-1 text-sm text-slate-500">{selected.stops.length > 0 ? "Ruta con paradas automáticas" : `Destino: ${selected.destination}`}</p><p className="mt-1 text-xs text-slate-500">GPS del conductor: {selected.driver?.name || selected.driver?.username || "Sin asignar"}</p></div>{view === "active" ? <div className="flex flex-wrap gap-2"><select value={selected.status} onChange={(event) => void updateStatus(event.target.value as Shipment["status"])} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold"><option value="READY">Listo</option><option value="IN_TRANSIT">En tránsito</option><option value="PAUSED">Pausado</option><option value="DELIVERED">Pedido entregado</option><option value="CANCELLED">Cancelado</option></select>{["READY", "IN_TRANSIT", "PAUSED"].includes(selected.status) ? <><button type="button" onClick={() => void updateStatus("DELIVERED")} className="rounded-xl bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-700">Terminar viaje</button><button type="button" onClick={() => void updateStatus("CANCELLED")} className="rounded-xl bg-rose-100 px-3 py-2 text-xs font-bold text-rose-700">Cancelar viaje</button></> : null}</div> : null}</div>
        {view === "history" ? <div className="mb-3 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Inicio GPS</p><p className="mt-1 text-sm font-bold text-slate-900">{formatDateTime(selected.startedAt)}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Final del envío</p><p className="mt-1 text-sm font-bold text-slate-900">{formatDateTime(selected.deliveredAt)}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Puntos GPS</p><p className="mt-1 text-sm font-bold text-slate-900">{selected.locations.length}</p></div></div> : null}
        {selected.stops.length > 0 ? <div className="mb-3 rounded-2xl border border-blue-100 bg-blue-50 p-4"><div className="flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[0.12em] text-blue-700">{view === "history" ? "Detalle de paradas" : "Paradas automáticas"}</p>{displayedRoute?.returnToOrigin ? <span className="text-xs font-bold text-emerald-700">Regreso a Yacelltech</span> : null}</div><div className="mt-3 grid gap-2 sm:grid-cols-2">{selected.stops.map((stop) => { const routeStop = displayedRoute?.stops.find((item) => item.id === stop.id); return <div key={stop.id} className="rounded-xl bg-white p-3"><div className="flex items-start justify-between gap-2"><p className="text-sm font-bold text-slate-900">{stop.name}</p><span className={`text-[10px] font-bold ${stop.status === "ARRIVED" ? "text-emerald-700" : "text-blue-700"}`}>{stop.status === "ARRIVED" ? `Llegó ${formatDateTime(stop.arrivedAt)}` : routeStop ? `${routeStop.distanceKm} km · ${routeStop.durationMinutes} min` : "Pendiente"}</span></div><p className="mt-1 text-xs text-slate-500">{stop.address}</p>{view === "history" && stop.arrivedAt ? <p className="mt-2 text-xs font-semibold text-emerald-700">Hora de llegada: {formatDateTime(stop.arrivedAt)}</p> : null}</div>; })}</div></div> : null}
        ...
        <ShipmentMap shipment={selected} route={view === "active" ? displayedRoute : null} />{activeSelected ? <GpsTracker shipmentId={selected.id} active={selected.driver?.id === currentUserId && ["READY", "IN_TRANSIT", "PAUSED"].includes(selected.status)} destination={autoDestination} onDelivered={() => removeTerminalShipment(selected.id)} /> : null}
