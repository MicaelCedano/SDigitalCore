    ...
    <main className="mx-auto max-w-[1500px] space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">{selected ? <>
          ...
          <ShipmentMap shipment={selected} route={displayedRoute} /><GpsTracker shipmentId={selected.id} active={selected.driver?.id === currentUserId && ["READY", "IN_TRANSIT", "PAUSED"].includes(selected.status)} destination={displayedRoute?.destination ?? null} onDelivered={() => removeTerminalShipment(selected.id)} />
