...
export type RouteStop = { id: string; name: string; address: string; status: "PENDING" | "ARRIVED"; latitude: number; longitude: number; distanceKm: number; durationMinutes: number; coordinates: [number, number][]; destination: { latitude: number; longitude: number; label: string } };
export type PlannedRoute = { coordinates: [number, number][]; distanceKm: number; durationMinutes: number; destination: { latitude: number; longitude: number; label: string }; stops: RouteStop[]; returnToOrigin: boolean };
...
export function ShipmentMap({ shipment, route }: { shipment: Shipment; route: PlannedRoute | null }) {
  ...
  const stopRoutes = route?.stops ?? [];
  return <div className="relative h-[500px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-inner"><MapContainer center={center} zoom={10} scrollWheelZoom className="h-full w-full"><TileLayer attribution='&copy; OpenStreetMap contributors &copy; CARTO' url="https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png" /><FitRoute shipment={shipment} route={route} />{route ? <Polyline positions={route.coordinates} pathOptions={{ color: "#2563eb", weight: 8, opacity: 0.95, lineCap: "round", lineJoin: "round" }} /> : null}{stopRoutes.slice(1).map((stop) => <Polyline key={stop.id} positions={stop.coordinates} pathOptions={{ color: "#93c5fd", weight: 4, opacity: 0.65, dashArray: "7 8" }} />)}{actualPoints.length > 1 ? <Polyline positions={actualPoints} pathOptions={{ color: "#0f172a", weight: 5, opacity: 0.85, lineCap: "round", lineJoin: "round" }} /> : null}{route ? <Marker position={[route.destination.latitude, route.destination.longitude]} icon={destinationIcon}><Popup><strong>{route.returnToOrigin ? "Regreso a Yacelltech" : "Próxima parada"}</strong><br />{route.destination.label}</Popup></Marker> : null}{stopRoutes.map((stop) => <Marker key={stop.id} position={[stop.latitude, stop.longitude]} icon={destinationIcon}><Popup><strong>{stop.name}</strong><br />{stop.status === "ARRIVED" ? "Parada completada" : `${stop.distanceKm} km · ${stop.durationMinutes} min`}</Popup></Marker>)}{shipment.lastLocation ? <Marker position={[shipment.lastLocation.latitude, shipment.lastLocation.longitude]} icon={vehicleIcon}><Popup><strong>{shipment.code}</strong><br />Furgoneta en ruta<br />Última posición: {new Date(shipment.lastLocation.recordedAt).toLocaleString("es-DO")}</Popup></Marker> : null}</MapContainer><div className="pointer-events-none absolute left-4 top-4 z-[1000] max-w-[min(350px,calc(100%-2rem))] rounded-2xl bg-white/95 px-4 py-3 shadow-xl backdrop-blur"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-blue-600">{route?.returnToOrigin ? "Regreso final" : "Paradas pendientes"}</p><p className="mt-1 truncate text-sm font-black text-slate-900">{route?.destination.label ?? shipment.destination}</p>{route ? <div className="mt-2 flex gap-4"><div><p className="text-[10px] text-slate-500">Próximo tramo</p><p className="text-lg font-black text-slate-900">{route.durationMinutes >= 60 ? `${Math.floor(route.durationMinutes / 60)} h ${route.durationMinutes % 60} min` : `${route.durationMinutes} min`}</p></div><div><p className="text-[10px] text-slate-500">Distancia</p><p className="text-lg font-black text-slate-900">{route.distanceKm} km</p></div></div> : <p className="mt-1 text-xs text-slate-500">Activa el GPS para dibujar la ruta.</p>}</div></div>;
}
